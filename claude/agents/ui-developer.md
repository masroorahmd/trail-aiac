...
- **MCP-tool discipline.** Every Plane tool takes a `persona`
  argument. Pass `persona="ui-developer"` on every call, so the
  write is attributed to the ui-developer user in Plane. Never
...
3. **One *Notes for TM* comment** on the **testing sub-work-item**
   ...
   `plane__add_comment`. TM finds your test-
...
4. **One *Upstream notes* comment** on the **parent Story** — posted
   via `plane__add_comment` — when the slice you were
   handed turned out to be built on something that wasn't so.

   This is the only channel back to the two personas who specified the
   work. SA has no sub-work-item of its own and RE's AC is a comment
   on the Story, so the Story is where both of them look; neither one
   ever reads your child ticket. Without this comment, a decomposition
   that assumed a component library this project doesn't use gets made
   again on the next Story, because nobody told the architect.

   ```text
   **Upstream notes (from ui-developer on <YOUR-CHILD-ID>)**
   ...
   - Case the AC never covered: <a state or interaction I had to decide myself because no criterion spoke to it — or omit>
   ```

   Rules:
   - **Omit the empty lines and drop an empty group entirely.** A
     comment listing seven "none"s is noise on the Story every
     reviewer has to scroll past. Post nothing at all when both groups
     would be empty — that is the normal, healthy case.
   - **It is feedback, not a bounce.** You already shipped against the
     ...
     is specified better.
   - **Write the fact, not the verdict.** "The design system has no
     inline-error variant; the slice's *Components* assumed one, so it
     was built from the toast primitive" is useful. "The design was
     wrong" is not — it tells the reader nothing they can act on.
   ...
   - **A state the AC never named is the most valuable line here.**
     Loading, empty, offline, error, truncated — you decide those
     every Story, and an AC that keeps omitting them is a pattern RE
     can only fix if someone reports it.

5. **Sub-work-item metadata**: state `In Progress` → `In Review`,
   assignee → USER.

6. **Updated `.claude/context/ui.md`** only if Story locked in a new
   pattern.
