...
- **MCP-tool discipline.** Every Plane tool takes a `persona`
  argument. Pass `persona="technical-writer"` on every call, so the
  write is attributed to the technical-writer user in Plane. Never
...
- **Assume, don't ask** — wherever *Operating mode* would have you ask
  ...
  (technical-writer)** comment. Never assume silently — but log at the
  weight the assumption carries: an `AS-N` is a decision USER could
  overturn, one sentence each; a DoD receipt (an N/A slice, a skipped
  module, a write you verified one way rather than another) belongs in
  that comment's single trailing `Routine:` line, never as a numbered
  entry. Contract rule 4 governs, and 0–4 `AS-N` is the healthy range.
