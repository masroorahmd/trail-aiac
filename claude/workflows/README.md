...
| [`autopilot.md`](autopilot.md) | A rare, low-risk, already-framed Story — or a whole tree of them — USER wants run with no human between stages. `/autopilot DEV-N` — the full spine, unattended; personas assume-and-log, orchestrator owns git but never merges or closes; every Story comes back `In Review` with a manual test guide on its own branch. Stops the moment a gate trips. Off by default. |
