...
11. **container hand-back** — after the last Story, the containers from
    ...
    to merge them.
**No retro stage.** BD, UD, SR and TM still leave ***Upstream notes***
comments whenever a slice's contract or an `AC-N` did not hold, and the
terminal summary lists them per Story — but autopilot never spawns SA
or RE to read them. The retro stays a **manual** turn USER decides is
worth taking: `/sa retro <STORY-ID>`, `/re retro <STORY-ID>`. Run
automatically it fired on nearly every Story, which made it ceremony
rather than signal; the notes keep, and reading several Stories' worth
in one turn is the better read anyway.
