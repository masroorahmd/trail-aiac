...
- **At or under __LARGE_FILE_LINES__ lines: read the whole file.**
  A partial read there saves nothing and only narrows your view.
- **Over __LARGE_FILE_LINES__ lines: read the head first, then the
  symbol.** The head is the module docstring plus the imports —
  roughly the first __HEAD_LINES__ lines — and it is not optional.
  A codebase states its invariants once, at the top, far from where
  they bind.
