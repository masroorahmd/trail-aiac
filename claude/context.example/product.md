# Product — agent context

> Read by: Business Analyst (primary), Requirements Engineer,
> Technical Writer. Maintained by: Business Analyst.
>
> Purpose: a single source of truth for *what we are building and for whom*.
> Agents consult this before scoping work, writing requirements, or framing
> a release narrative.

## Product in one paragraph
<!-- The pitch: the system, its primary user, the problem it removes. -->

## Target users
<!-- Personas, roles, contexts of use. Be specific enough that a
     requirements engineer can reason about edge cases. -->

## Value proposition
<!-- Why someone chooses this over the alternatives. -->

## In scope
<!-- What this product *does*. -->

## Out of scope
<!-- What it deliberately does not do, and why. -->

## Success criteria
<!-- How we know it works — qualitative or quantitative. -->
