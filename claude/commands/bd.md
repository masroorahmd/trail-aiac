---
description: Put the main loop into the backend-developer role to implement the backend slice of a Story.
argument-hint: "<sub-work-item to implement, e.g. DEV-N.backend>"
---

You are running `/bd` directly in the **main loop** of this
Claude Code session. Do **NOT** delegate to a subagent — `/bd`
puts you (the main loop) into the **backend-developer** role for this and
...
1. `.claude/agents/backend-developer.md` — the persona definition. Pay
   ...
   carries `persona="backend-developer"`), chat-first /
...
backend-developer role — do not pre-process, summarise, or split into tasks
on its behalf.
