...
| <img src="../avatars/ui-developer.png" width="60"/>          | `ui-developer`          | `/ud` | Implements the `frontend`-module sub-work-item; visually verifies **every** route the change touched in a browser before handing back (console + network swept on each), and enumerates them in the Implementation notes; posts an *Upstream notes* comment on the Story when the slice's contract or an `AC-N` did not hold. | After SR — `/ud <SUBTASK-ID>`. Also runs the pre-ticket design lane — `/mock "<screen or flow>"`. |
...
| <img src="../avatars/release-manager.png" width="60"/>       | `release-manager`       | `/rm` | Drives versioning, tagging, and release, and lands a finished ticket's branch (rebase + fast-forward) on the integration branch. Runs outside the Story workflow. | When you're cutting a release, or when a ticket you set to `Done` needs its branch landed — `/rm`. |
...
- **Shared skills** for anything two personas would otherwise encode
  ...
  TM's review run and UD's visual verification gate both follow. Plus
  [`ui-mockup`](../claude/skills/ui-mockup/SKILL.md) — the `/mock`
  design-session contract: where a mock lives, its fidelity, the `D-N`
  decision record, and how BA, RE, SA and UD each read the folder it
  leaves behind.
