...
| <img src="../avatars/test-manager.png" width="60"/>          | `test-manager`          | `/tm` | Implements the `testing`-module sub-work-item; owns test strategy and verification across the Story. Also drives an already-posted manual test guide in a live browser on demand, filing a *Rework request* on the owning persona's sub-work-item per defect. | After SR — `/tm <SUBTASK-ID>`. To run a guide: `/tm run manual test guide for <STORY-ID>`. |
